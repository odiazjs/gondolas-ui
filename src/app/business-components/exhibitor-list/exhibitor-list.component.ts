...
@Component({
    ...
    styleUrls: ['./exhibitor-list.component.scss']
})

export class ExhibitorListComponent implements OnInit, AfterViewInit {
    displayedColumns = ['nombre', 'categoria', 'marcas', 'tipo', 'actions'];
    ...
    addNew() {
      this.store.dispatch(new Navigate(['/exhibitor']))
    }

    startEdit(row: any) {
      this.store.dispatch(new Navigate([`/exhibitor/${row.id}`]))
    }
    ...
    connect(): Observable<any[]> {
        ...
        return merge(...displayDataChanges).pipe(map(() => {
            ...
            this.filteredData = this._exampleDatabase.data.slice().filter((item: any) => {
                if (item && (item.name || item.category || item.gondolaType.name)) {
                    const searchStr = (
                        item.name +
                        item.category +
                        item.gondolaType.name
                    ).toLowerCase();
